/**
 * This is the backup file for Yappacino.
 * The source code written in JavaScript.
 * The file is meant to be used as a reference, or as a backup in case the main index.yap fails beyond recovery.
...
const KEYWORDS = {
    "constant": (line) => {
        ...
        if (variable === null) throw new Error("Missing type on line: " + line)
    ...
    "mutable": (line) => {
        ...
        if (!variable[1]) console.log("Missing type on line: " + line)
        _TYPES_VARIABLES.set(variable[1], {
            "constant": false,
            is_volatile,
            type: variable[2]
...
async function main() {
    ...
    for (const filePath of files) {
        ...
        for (const compeer of compeers) {
            let body = compeer.replace(/^\s*compeer\s*/gm, "switch").trim();
            ...
            contents = contents.replace(compeer, PARTS.join("\n"))
        }

        const towards = contents.match(REGEX.towards) || [];
